...
public class Check {
    ...
    boolean checkSpecialSymbolBack(){
        ...
        if(perChar=='+'||perChar=='-'||perChar=='X'||perChar=='÷') return true;
        return false;
    }
    ...
    boolean checkNumberInput(){
        ...
        if(length>1){
            ...
            if((perPerChar=='+'||perPerChar=='-'||perPerChar=='X'||perPerChar=='÷'||perPerChar=='^')&&perChar=='0'){
    ...
    boolean checkPointInput(){
        ...
        char perChar = equation.charAt(length-1);
        //自动补0
        if(perChar=='+'||perChar=='-'||perChar=='X'||perChar=='÷'||perChar=='%') equation+="0";
        ...
        for(int i = length-1;i>=0;i--){
            ...
            if(checkChar=='+'||checkChar=='-'||checkChar=='X'||checkChar=='÷'||checkChar=='%') break;
        }
        if(pointNum>0) return false;
        return true;
    }
    /**
     * 输入+ - X / 时进行检查，在他们的前面不能有 .和%
     * */
    boolean checkOperationsInput(){
        ...
        //替换
        if(perChar=='+'||perChar=='-'||perChar=='X'||perChar=='÷')
            equation = equation.substring(0,length-1);
        return true;
    }
    /**
    ...
    boolean checkInvolutionInput(){
        ...
        for(int i = length-1;i>0;i--){
            ...
            if(iChar=='^') iNum++;
            if (iChar=='+'||iChar=='-'||iChar=='X'||iChar=='÷'||iChar=='%') break;
        }
    ...
    boolean checkLBracketInput(){
        ...
        if(length==0) return true;
        char perChar = equation.charAt(length-1);
        if(perChar=='+'||perChar=='-'||perChar=='X'||perChar=='÷'||perChar=='%'||perChar=='^'||perChar=='(') return true;
        return false;
    }
    /**
     * 输入右括号
    ...
    boolean checkFactorialInput(){
        ...
        if(perChar=='!'||perChar=='%'||perChar=='^'||perChar=='('||perChar=='+'
                ||perChar=='-'||perChar=='X'||perChar=='÷') return false;
        //小数点阶乘不可以存在
        for(int i=length-1;i>=0;i--){
            char checkChar = equation.charAt(i);
            if(checkChar=='.') return false;
            if(checkChar=='+'||checkChar=='-'||checkChar=='X'||checkChar=='÷') break;
        }
        return true;
    }
}
